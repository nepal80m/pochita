...
This sample provides an environment to display and capture metrics from the testnet in real time. It consists of a docker-compose file that starts a Prometheus and Grafana server setup configured to collect and display metrics for the testnet.
...
You will need to have installed **docker-compose with version 1.29 or above** (note that this is higher than the v1.14 requirement requested for the testnetwork).

## How to use

1. Go to the testnet directory and run bring up the test-network **./network.sh up createChannel**
